...
#Importing here has no harm as it is always called after freezing the config file
...
class ResHead(nn.Module):
    ...
    def __init__(self, w_in, nc, isPenultimateMode=False):
        super(ResHead, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(w_in, nc, bias=True)
        self.isPenultimateMode = isPenultimateMode

    def forward(self, x):
        x = self.avg_pool(x)
        z = x.view(x.size(0), -1)
        x = self.fc(z)
        if self.isPenultimateMode:
            return z,x
        return x
...
class ResNet(nn.Module):
    ...
    def __init__(self, temp_cfg):
        ...
        self.description = "Open source implementation of Resne(x)t adapted from pycls official repository"

        self.source_link = "https://github.com/facebookresearch/pycls/blob/master/pycls/models/resnet.py"
        ...
        print("if condition {}".format(cfg.TRAIN.DATASET in ['CIFAR10','CIFAR100','SVHN']))
        if cfg.TRAIN.DATASET in ['CIFAR10','CIFAR100','SVHN']:
    ...
    def _construct_cifar(self):
        assert (self.model_depth - 2) % 6 == 0, \
            f'Model depth should be of the format 6n + 2 for cifar. Currently depth is {self.model_depth}'
        logger.info('Constructing: ResNet-{}'.format(self.model_depth))
        # Each stage has the same number of blocks for cifar
        d = int((self.model_depth - 2) / 6)
        ...
        # Added isPenultimateMode Flag to get penultimate layer code
        self.head = ResHead(w_in=64, nc=cfg.MODEL.NUM_CLASSES, isPenultimateMode=self.penultimate_active)

    def _construct_imagenet(self):
        logger.info('Constructing: ResNe(X)t-{}-{}x{}, {}'.format(
            self.model_depth,
        ...
        (d1, d2, d3, d4) = _IN_STAGE_DS[self.model_depth]
        ...
        self.head = ResHead(w_in=2048, nc=cfg.MODEL.NUM_CLASSES, isPenultimateMode=self.penultimate_active)

    def forward(self, x):
        if self.penultimate_active:
            #penultimate_mode is active
            self.head.isPenultimateMode = True
        else:
            self.head.isPenultimateMode = False
